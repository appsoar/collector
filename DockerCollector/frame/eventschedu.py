...
from frame.errcode import FAIL, CLOUD_BOOT_TASK_FAIL
...
class EventSchedu(Event):
    ...
    def get_state(self):
        if self.is_success():
            return Result(100)
        if self.is_fail():
            return Result(0,CLOUD_BOOT_TASK_FAIL,self.get_log())
        
        progress = 0
        for event in self.__event_list.values():
            if event.is_fail():
                return Result(0,CLOUD_BOOT_TASK_FAIL,event.get_log())
            else:
